...
<template>
  <div class="bg-neutral-200 dark:bg-neutral-900 rounded-md p-5 px-32 flex flex-col items-center space-y-2">
    <div v-if="!pending && !error" class="flex justify-end items-end">
      <nuxt-img v-if="data.data.discord_user.avatar" draggable="false" :src="`https://cdn.discordapp.com/avatars/${data.data.discord_user.id}/${data.data.discord_user.avatar}.${data.data.discord_user.avatar.startsWith('a_') ? 'gif' : 'webp'}?size=128`" :alt="`${data.data.discord_user.username}#${data.data.discord_user.discriminator}`" class="relative w-24 h-24 rounded-full"></nuxt-img>
      <nuxt-img v-else draggable="false" src="https://cdn.discordapp.com/embed/avatars/5.png" :alt="`${data.data.discord_user.username}#${data.data.discord_user.discriminator}`" class="relative w-24 h-24 rounded-full"></nuxt-img>
